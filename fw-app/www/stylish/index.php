...
<body>
	...
	<?php
		...
		if(!isset($_COOKIE["geolocation"])){
			$loc_info = json_decode(file_get_contents('http://api.ipinfodb.com/v3/ip-city/?key=6a6abc81b02243f15afbce31ee0cc764d84c79903a4496309e215f8b5ad578da&ip='.$ip.'&format=json'));
			if($loc_info->cityName != "-")$location = $loc_info->cityName.", ".$loc_info->countryName;
			 setcookie("geolocation", $location, time()+3600*24*7); //set cookie for 1 week
		}else{
			$location = $_COOKIE["geolocation"];
		}
		
		$cache_f = 'photos_'.strtolower(str_replace(array(" ",",","."),"",$location));
		if(!APP()->cache->has($cache_f)) {
			$tags = "architecture,buildings,panoramic";
			$imgs = json_decode(file_get_contents('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=6467c0a0bda9e3640c8b3340c1036969&tags='.urlencode($tags).'&text='.urlencode($location).'&format=json&nojsoncallback=1&tag_mode=all&extras=url_o,url_l'));
			APP()->cache->set($cache_f,$imgs->photos->photo);
		}
		$imgs = APP()->cache->get($cache_f);
		$src = get_rand($imgs);
		
		function get_rand($photos){
			$rand = rand(0,count($photos)-1);
			$img = $photos[$rand];
			return (isset($img->url_l))?$img->url_l:get_rand($photos);
		}
		
	?>
    <div id="top" class="header" style="max-height:500px; overflow:hidden; background:none;">
		<div id="bg-top" style="position:absolute; top:0; left:0; width:100%; max-height:100%;"><img src='<?php echo $src; ?>' alt='<?php echo $location; ?>' style="width:100%;" /></div>
        <div class="vert-text" style="position:relative; z-index:2;">
